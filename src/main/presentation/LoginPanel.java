...
public class LoginPanel extends javax.swing.JFrame {
    private final PresentationController CP;
     ...
     */
    LoginPanel(PresentationController CP) {
        this.CP = CP;
        initComponents();
    ...
    private void helpActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            // 1. Busquem el fitxer directament a la carpeta docs amb el nom nou
            File pdfFile = new File("docs/User_Manual.pdf");
            System.out.println("Buscant el PDF a: " + pdfFile.getAbsolutePath());

            // 2. Comprovem si el fitxer realment existeix
            if (pdfFile.exists()) {
                // 3. Comprovem si el sistema operatiu permet obrir fitxers
                if (Desktop.isDesktopSupported()) {
                    Desktop desktop = Desktop.getDesktop();
                    desktop.open(pdfFile);
                } else {
                    System.err.println("Error: La classe Desktop no és compatible amb aquest sistema operatiu.");
                }
            } else {
                System.err.println("Error: No s'ha trobat el manual d'usuari a " + pdfFile.getAbsolutePath());
            }
        } catch (Exception e) {
            System.err.println("S'ha produït un error en intentar obrir el PDF:");
            e.printStackTrace();
        }
    }
